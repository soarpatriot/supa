...
-- This schema reflects all migrations applied up to 20251203130400
...
CREATE TABLE assets (
  ...
  created_at TIMESTAMP(3),
  updated_at TIMESTAMP(3),
  CONSTRAINT fk_assets_topic FOREIGN KEY (topic_id)
    REFERENCES topics(id) ON DELETE CASCADE,
  CONSTRAINT check_assets_type CHECK (type >= 0 AND type <= 4)
);

-- Asset type constants (for reference):
-- 0: picture
-- 1: video
